(() => {
  ...
  // src/workers/exportWorker-src.js
  init_polyfill_buffer();
  importScripts("/libs/shpwrite.js", "/libs/jszip.js");
  var DBFGenerator = class {
    ...
    analyzeSchema() {
      ...
      Object.keys(sampleProps).forEach((key) => {
        const values = this.features.map((f) => f.properties?.[key]).filter((v) => v !== void 0);
        const field = {
          name: this.normalizeFieldName(key),
          // Modificado para mantener original
          originalName: key,
    ...
    writeFieldDescriptor(buffer, offset, field) {
      ...
      if (nameBytes.length < 11) {
        buffer.set(new Uint8Array(11 - nameBytes.length).fill(0), offset + nameBytes.length);
      }
    ...
    encodeValue(value, field) {
      ...
      switch (field.type) {
        case "N":
          ...
          return this.padStart(encoder.encode(str), size, 32);
        ...
        default:
          ...
          if (length < size) {
            result.set(new Uint8Array(size - length).fill(32), length);
          }
          return result;
      }
    }
  ...
  self.onmessage = async (e) => {
    ...
    try {
      ...
      const validFeatures = geojson.features.filter((f) => {
        try {
          return f?.geometry?.coordinates && Array.isArray(f.geometry.coordinates) && f.geometry.coordinates.length > 0;
        } catch {
          return false;
        }
      });
      ...
      const extractFile = async (zip2, ext) => {
        const filename = Object.keys(zip2.files).find(
          (f) => f.toLowerCase().endsWith(ext.toLowerCase())
        );
        if (!filename) {
          throw new Error(`Required ${ext} file not found in generated ZIP`);
        }
        const file = zip2.file(filename);
        if (!file) {
      ...
      folder.file(`${baseName}.cpg`, encoding);
      folder.file(
        `${baseName}.prj`,
      ...
      self.postMessage({
        ...
        details: {
          inputValidation: {
            ...
            validFeatures: geojson?.features?.filter(
